<div>
                    ...
                    <div class="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                ...
                                <tr>
                                    <td class="px-6 py-4 whitespace-nowrap"> <div class="text-sm font-medium text-gray-900"> {{ $college->college }} </div>
                                    <td class="px-6 py-4 whitespace-nowrap"> <div class="text-sm font-medium text-gray-900"> {{ $college->created_at->ToFormattedDateString() }} </div>
                                    <td class="px-6 py-4 whitespace-nowrap"> <div class="text-sm font-medium text-gray-900"> {{ $college->updated_at->ToFormattedDateString() }} </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                                    <button wire:click="editOpenModal({{ $college->id}})" class="text-indigo-600 hover:text-indigo-900">Edit</button>
                                    <button wire:click="deleteOpenModal({{ $college->id}})" class="text-red-600 hover:text-red-900">Delete</button>
                                    </td>
                                </tr>
                                @empty
                                <tr>
            ...
            <x-slot name="content">
                <form wire:submit.prevent="create">
                    <div class="col-span-6">
                        ...
                        <input wire:model.lazy="college" type="text" placeholder="e.g. College of Computer Studies" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
        ...
        <x-jet-dialog-modal wire:model.defer="editModal">
            ...
            <x-slot name="content">
                <form wire:submit.prevent="update">
                    <div class="col-span-6">
                        <label class="block text-sm font-medium text-gray-700">College</label>
                        <input wire:model.lazy="college" type="text" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                        <x-jet-input-error for="college"/>
                    </div>
                </form>
            </x-slot>
    ...
    <x-jet-dialog-modal wire:model.defer="deleteModal">
        ...
        <x-slot name="content">
            {{ __('Are you sure you want to delete selected college?') }}
        </x-slot>
